...
## Construir desde un Clon Fresco

Si acabas de clonar el repositorio, sigue estos pasos para preparar el entorno de build:
...
# 2. Instalar dependencias de Node.js (para scripts de build)
npm install
...
pip install pyinstaller  # Requerido para empaquetar el backend

# 6. Ahora puedes construir la aplicación
npm run build:all
```
...
- electron-builder: se instala automáticamente con `npm install` en `gui-electron/`
- Herramientas del sistema:
  - `tar` y `unzip` (para extraer binarios de Ollama)
  - `make` y compiladores (para algunas dependencias de Python)
...
Este comando:
1. Descarga los binarios de Ollama para la plataforma actual (o todas si `DOWNLOAD_ALL_PLATFORMS=true`)
2. Empaqueta el backend Python con PyInstaller
3. Construye la aplicación Electron con todos los recursos

**Nota**: Por defecto solo descarga Ollama para la plataforma actual. Para descargar todas las plataformas:
```bash
DOWNLOAD_ALL_PLATFORMS=true npm run build:all
```
